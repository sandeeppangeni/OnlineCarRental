...
if(isset($_POST['submit']))
  {		$emailid=$_POST['emailid'];
	  	...
	  	$esql = "SELECT * from tblvehicles where DriverEmail=:emailid";
	  	$dsql = "SELECT * from tbldriver where EmailId=:emailid";
	  	$vsql = "SELECT * from tblvehicles where VehicleId=:vehicleid";
		$query = $dbh -> prepare($esql);
		$vquery = $dbh -> prepare($vsql);
		$dquery = $dbh -> prepare($vsql);

		$query -> bindParam(':emailid',$emailid, PDO::PARAM_STR);
		$vquery -> bindParam(':vehicleid',$vehicleid, PDO::PARAM_STR);
		$dquery -> bindParam(':emailid',$emailid, PDO::PARAM_STR);
		...
		$results=$vquery->fetchAll(PDO::FETCH_OBJ);
		$results=$dquery->fetchAll(PDO::FETCH_OBJ);
		if($query->rowCount() > 0)
		{
	  	  $error = "Oops... This email id has been already taken"; 	
		}
		elseif ($dquery->rowCount() > 0) {
			$error = "Oops... This email id is not registered as driver email id";
		}
		elseif ($vquery->rowCount() > 0) {
			$error = "Oops... This vehicle id has been already taken";
		}
	 	else
	 	{
			$vehicletitle=$_POST['vehicletitle'];
			$vehicleid=$_POST['vehicleid'];
			$emailidid=$_POST['emailid'];
			...
			$sql="update tblvehicles set VehiclesTitle=:vehicletitle,Vehicleid=:vehicleid,DriverEmail=:emailid,VehiclesBrand=:brand,VehiclesOverview=:vehicleoverview,PricePerDay=:priceperday,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats where id=:id ";
...
<label class="col-sm-2 control-label">Vehicle Id<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="vehicleid" class="form-control" value="<?php echo htmlentities($result->VehicleId)?>" required>
</div>

<label class="col-sm-2 control-label">Driver Email Id<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="emailid" class="form-control" value="<?php echo htmlentities($result->DriverEmail)?>" required>
</div>
</div>
